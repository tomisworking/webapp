...
**📋 Wybór API URL zależy od konfiguracji Cloudflare:**

**Opcja 1: Cloudflare Proxy WŁĄCZONE (Proxied) - ZALECANE**
- Użyj domeny: `http://kongoapp.pl/api` lub `https://kongoapp.pl/api`
- Cloudflare proxy'uje requesty do ALB

**Opcja 2: Cloudflare Proxy WYŁĄCZONE (DNS only)**
- Użyj domeny: `http://kongoapp.pl/api`
- DNS wskazuje na ALB, ale requesty idą przez domenę

**Opcja 3: Test bezpośrednio ALB (tylko do testów)**
- Użyj ALB DNS: `http://forum-alb-1684129147.us-east-1.elb.amazonaws.com/api`

**Windows PowerShell:**
```powershell
# Dla Cloudflare (proxy włączone lub wyłączone) - ZALECANE:
@"
REACT_APP_API_URL=http://kongoapp.pl/api
"@ | Out-File -FilePath .env.production -Encoding utf8

# LUB dla HTTPS (jeśli masz certyfikat SSL):
@"
...
- Używaj domeny (`kongoapp.pl`), nie ALB DNS bezpośrednio (chyba że testujesz)
- Django CORS jest skonfigurowany dla: ALB DNS, `kongoapp.pl`, `www.kongoapp.pl`

**Przykładowa zawartość `.env.production` (Cloudflare):**
```
REACT_APP_API_URL=http://kongoapp.pl/api
```
